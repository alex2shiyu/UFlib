...
#SBATCH --array=1-4%5              # Number of job array tasks
...
module load vasp/6.4.1_gpu-nvidia
...
cpus_per_task=1
#`expr $cpus_per_node / $mpi_tasks_per_node`
#OpenMP settings:  16 for GPU nodes, 32 for CPU nodes
# for qe, perturbo on nersc
...
for ((iC=$Cstart; iC<=$Cend; iC++)); do
    ...
    cp -r "../../iter.0000_almSample/disp${NQ_source}.POSCAR" ./"$DIR1"/POSCAR
    cp -r "../../../0_data/INCAR" ./"$DIR1"/INCAR
    cp -r "../../../0_data/KPOINTS" ./"$DIR1"/KPOINTS
    cp -r "../../../0_data/POTCAR" ./"$DIR1"/POTCAR
    cd $DIR1
   ...
   #    $EXEFILE -npools $npools_cpu -nband $nbands_cpu -i $INPUTFILE > $OUTPUTFILE
    srun -n $total_mpi_tasks -c $cpus_per_task -G $total_mpi_tasks --cpu-bind=cores --gpu-bind=none $EXEFILE > $OUTPUTFILE 2>err
