...
# BKC -->
    ...
    def save_qe_xml(self, support_xml_filename, filename, full_name = False):
        ...
        for iq in range(nqirr):
            ...
            tree = ET.parse(support_xml_filename+str(iq+1)+'.xml')
            root = tree.getroot()

            if iq + 1 == 1:
                diel_node = root.find('DIELECTRIC_PROPERTIES')

                epsil_flag = diel_node.get('epsil', 'false').lower() == 'true'
                zstar_flag = diel_node.get('zstar', 'false').lower() == 'true'

                if epsil_flag:

                    epsilon = root.find('DIELECTRIC_PROPERTIES/EPSILON')
                    epsilon.text = '\n'
                    for i in range(3):
                        epsilon.text += "  {: .15E}  {: .15E}  {: .15E}\n".format(*list(self.dielectric_tensor[:,i]))
                    epsilon.text += '    '
                else:
                    print("⚠️ Skipping EPSILON section (epsil=false)")

                if zstar_flag:
                    zstar = root.find('DIELECTRIC_PROPERTIES/ZSTAR')
                    assert len(zstar)==n_atoms, '#zstar != #atoms'
                    for i in range(self.structure.N_atoms):
                        cur = zstar.find('Z_AT_.'+str(i+1))
                        cur.text = '\n'
                        for j in range(3):
                            cur.text += "  {: .15E}  {: .15E}  {: .15E}\n".format(*list(self.effective_charges[i, :, j]))
                        cur.text += '    '
                else:
                    print("⚠️ Skipping ZSTAR section (zstar=false)")


            for jq, qr in enumerate(self.q_stars[iq]):

                dyn = root.find('DYNAMICAL_MAT_.'+str(jq+1))
                qrxml = [float(x) for x in dyn.find('Q_POINT').text.split()]
                assert abs(qr[0]*self.alat - qrxml[0]) < 10**-8, "Q_POINT mismatch!"
                assert abs(qr[1]*self.alat - qrxml[1]) < 10**-8, "Q_POINT mismatch!"
                assert abs(qr[2]*self.alat - qrxml[2]) < 10**-8, "Q_POINT mismatch!"

                for i in range(n_atoms):
                    for j in range(n_atoms):
                        phi = dyn.find("PHI."+str(i+1)+"."+str(j+1))
                        phi.text = '\n'
                        phi.text += "  {: .15E}  {: .15E}\n".format(np.real(self.dynmats[count_q][3*i+0, 3*j+0]), np.imag(self.dynmats[count_q][3*i+0, 3*j+0]))
                        phi.text += "  {: .15E}  {: .15E}\n".format(np.real(self.dynmats[count_q][3*i+1, 3*j+0]), np.imag(self.dynmats[count_q][3*i+1, 3*j+0]))
                        phi.text += "  {: .15E}  {: .15E}\n".format(np.real(self.dynmats[count_q][3*i+2, 3*j+0]), np.imag(self.dynmats[count_q][3*i+2, 3*j+0]))
                        phi.text += "  {: .15E}  {: .15E}\n".format(np.real(self.dynmats[count_q][3*i+0, 3*j+1]), np.imag(self.dynmats[count_q][3*i+0, 3*j+1]))
                        phi.text += "  {: .15E}  {: .15E}\n".format(np.real(self.dynmats[count_q][3*i+1, 3*j+1]), np.imag(self.dynmats[count_q][3*i+1, 3*j+1]))
                        phi.text += "  {: .15E}  {: .15E}\n".format(np.real(self.dynmats[count_q][3*i+2, 3*j+1]), np.imag(self.dynmats[count_q][3*i+2, 3*j+1]))
                        phi.text += "  {: .15E}  {: .15E}\n".format(np.real(self.dynmats[count_q][3*i+0, 3*j+2]), np.imag(self.dynmats[count_q][3*i+0, 3*j+2]))
                        phi.text += "  {: .15E}  {: .15E}\n".format(np.real(self.dynmats[count_q][3*i+1, 3*j+2]), np.imag(self.dynmats[count_q][3*i+1, 3*j+2]))
                        phi.text += "  {: .15E}  {: .15E}\n".format(np.real(self.dynmats[count_q][3*i+2, 3*j+2]), np.imag(self.dynmats[count_q][3*i+2, 3*j+2]))
                        phi.text += '    '
            ...
            frequencies_thz_cmm1 = root.find('FREQUENCIES_THZ_CMM1')

            for mu in range(len(freqs)):
                omega = frequencies_thz_cmm1.find('OMEGA.'+str(mu+1))
                omega.text = "\n  {: .15E}  {: .15E}\n    ".format(freqs[mu]*RyToTHz, freqs[mu]*RY_TO_CM)
                displacement = frequencies_thz_cmm1.find('DISPLACEMENT.'+str(mu+1))
                displacement.text = "\n"
                for i in range(n_atoms):
                    displacement.text += " {: .15E} {: .15E}\n".format(np.real(atomic_disp[3*i+0,mu]), np.imag(atomic_disp[3*i+0,mu]))
                    displacement.text += " {: .15E} {: .15E}\n".format(np.real(atomic_disp[3*i+1,mu]), np.imag(atomic_disp[3*i+1,mu]))
                    displacement.text += " {: .15E} {: .15E}\n".format(np.real(atomic_disp[3*i+2,mu]), np.imag(atomic_disp[3*i+2,mu]))
                displacement.text += "    "

            ET.indent(tree, space="  ", level=0)
            tree.write(filename+str(iq+1)+'.xml', xml_declaration=True, encoding="utf-8")
