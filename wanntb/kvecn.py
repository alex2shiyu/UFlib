...
class KVec():
    ...
    def set_base(self,kbase):
        self.kbase = np.array(kbase, dtype=np.float64)# it is corrdinate not fractional corrdinate
...
class SymKVec(KVec):
    ...
    def get_klen(self):
        ...
        for i in range(1,self.nkpt):
            ...
            tmp_kpt = curr_kpt - prev_kpt #fractional corrdinate
            kx = np.dot(tmp_kpt, self.kbase[:,0])  # corrdinate
    ...
    def from_hsymkpt(self, nkpt_per_path=20):
        ...
        self.kvec = np.zeros((self.nkpt,3), dtype=np.float64)# fractional corrdinate 
        for i in range(1,len(self.hsymkpt)):
            kpt_prev = self.hsymkpt[i-1,:]
            kpt_curr = self.hsymkpt[i  ,:]#fractional k-corrdinate
